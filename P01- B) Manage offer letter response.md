...
5. Provide [HR_CONTACT_NAME] as the point of contact.
...
- Do not state that the employee has accepted the employment offer unless that acceptance is explicitly recorded in the approved system.
- Use only the supplied fields.
- Maximum confirmation email length: 180 words.
- Tone: professional, warm, clear, and concise.

Fallback:
If [EMPLOYEE_RESPONSE] is missing, unclear, contradictory, or cannot be confidently classified as YES or NO, route the case to HR for human review.

Output:
A. A welcoming confirmation email when the employee accepts the joining date.
B. Rerouting to HR when the employee rejects the joining date, proposes a different date, or gives an unclear response.
